...
%            end 
          ...
          SimpleTress01a2=((sqrt(V1)*M0+sqrt(V0)*M1)/(sqrt(V1)+sqrt(V0)))+(sqrt(V1)*(V1-V0)/(2*sqrt(V0)*(M1-M0)));
           SimpleTress12a2=((sqrt(V1)*M2+sqrt(V2)*M1)/(sqrt(V1)+sqrt(V2)))+(sqrt(V2)*(V2-V1)/(2*sqrt(V1)*(M2-M1)));
           SimpleTress23a2=((sqrt(V2)*M3+sqrt(V3)*M2)/(sqrt(V3)+sqrt(V2)))+(sqrt(V3)*(V3-V2)/(2*sqrt(V2)*(M3-M2)));
...
%                        end 
      ...
      if(mod(packets,HIST_SIZE) ==0)
           ...
           infomatr(7)= sum(BER_sum)/((correct_packets+error_packets)*length(fixedpacketdata));
            ...
            %save('FM_amb_98_5_Mhz_100_packets_1Msps_v2','dataset')
            save('4PAM_FM_CW_min10_500_pakets_1MSps_normal_thress','infomatr')
             return;
      end
