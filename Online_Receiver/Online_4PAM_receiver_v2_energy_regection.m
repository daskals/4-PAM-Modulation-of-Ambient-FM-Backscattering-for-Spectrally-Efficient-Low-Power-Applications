...
while (1)
   ...
   %% Sync via ENERGY
         ...
         if DEBUG_en1==1;
                     ...
                     F_sensor_est_power=20*log10((abs(x_fft).^2)*Ts/50*1e3)-10; 
...
%                        end 
      ...
      if(mod(packets,HIST_SIZE) ==0)
           ...
           infomatr(7)= sum(BER_sum)/((correct_packets+error_packets)*length(fixedpacketdata));
            ...
            %save('4PAM_FM_95_8_min20_100_pakets_0_20m_1MSps_dataset','dataset')
            %save('NOISE_100_pakets_1MSps_dataset','dataset')
            %save('4PAM_FM_95_8_min20_100_pakets_0_20m_1MSps_info','infomatr')
            save('4PAM_FM_95_8_min20_6000_pakets_0_10cm_1MSps_info_v2','infomatr')
             return;
      end
